...
def build_gateway_handler(config: dict[str, Any]) -> type[BaseHTTPRequestHandler]:
    class GuardGatewayHandler(BaseHTTPRequestHandler):
        ...
        def _open_upstream(self, body: bytes):
        ...
        def _forward_upstream(self, body: bytes) -> tuple[int, list[tuple[str, str]], bytes]:
            response = self._open_upstream(body)
            try:
...
def run_wizard(_: argparse.Namespace) -> int:
    ...
    print_title(
        "AI / Agent 接入保护平台向导",
        "默认按 Runtime 注册审批流执行：输入平台地址和注册码，自动注册、轮询审批、落地长期凭据，再生成本地前置防护网关。",
    )
    print("你现在要做的是：告诉脚本“保护平台在哪、注册码是什么、上游 Agent 在哪、业务侧最终准备改成访问哪个网关地址”。")
    ...
    enrollment_token = prompt_secret("注册码 / Enrollment Token")
    verify_platform_tls = prompt_yes_no("是否校验保护平台 HTTPS 证书", True)

    print()
    print("3/10 下面填写上游 Agent 信息。")
    upstream_base_url = normalize_base_url(prompt_text("上游 Agent 地址", "http://AGENT_HOST:4567"))
    ...
    try:
        ...
        print()
        print(f"[错误] Runtime 注册或审批失败：{exc}")
        print(f"当前配置已保留：{config_path}")
        print("如果管理员稍后审批，可直接运行 validate 或 run 继续完成凭据落地。")
        return 1
...
def quick_openclaw(args: argparse.Namespace) -> int:
    ...
    print_title("OpenClaw 快速接入", "按最少参数生成前置防护网关，并自动走 Runtime 注册审批流。")
    ...
    try:
        config = register_runtime_flow(
            config_path,
            config,
            enrollment_token=str(args.enrollment_token),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    quick_openclaw_parser.add_argument("--enrollment-token", required=True, help="一次性 Runtime 注册码")
    quick_openclaw_parser.add_argument("--upstream-base-url", required=True, help="上游 OpenClaw 地址，例如 http://OPENCLAW_HOST:18789")
