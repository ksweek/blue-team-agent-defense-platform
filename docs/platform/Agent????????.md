...
.\connect_agent_gateway.cmd quick-openclaw `
  ...
  --upstream-base-url http://OPENCLAW_HOST:18789 `
  --upstream-token <openclaw-token> `
  --listen-port 9010 `
  --access-host <gateway-access-host>
```
...
.\connect_openclaw_control.cmd `
  --upstream-http-url http://OPENCLAW_HOST:18789 `
...
.\connect_openclaw_control.cmd `
  ...
  --platform-base-url http://127.0.0.1:8000 `
  --enrollment-token <一次性注册码> `
  --runtime-display-name openclaw-control-OPENCLAW_HOST-18789 `
  --target-agent-name OpenClaw-控制台-OPENCLAW_HOST `
  --review-action block `
  --listen-port 19090 `
  --access-host 127.0.0.1 `
  --log-jsonl .\run_logs\openclaw-control-frames.jsonl
...
请直接打开这个本地地址，而不要再直连远端 `OPENCLAW_HOST:18789`。这样：
