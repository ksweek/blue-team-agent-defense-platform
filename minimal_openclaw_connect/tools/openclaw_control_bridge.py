...
def extract_paths_from_text_values(*values: Any) -> list[str]:
    paths: list[str] = []
    for value in values:
        if value is None:
            continue
        if isinstance(value, str):
            ...
            text = stringify_message_content(value)
        elif isinstance(value, (list, tuple, set)):
            paths.extend(extract_paths_from_text_values(*list(value)))
            continue
        else:
            text = str(value)
        if not text:
        ...
        for pattern in (WINDOWS_PATH_RE, UNIX_PATH_RE):
            for match in pattern.finditer(text):
                candidate = clean_extracted_path(match.group(0))
                if candidate:
...
def context_has_security_surface_signal(context: dict[str, Any]) -> bool:
    return any(
        [
            context.get("paths") or [],
            context.get("requested_scopes") or [],
            context_has_tool_signal(context),
        ]
    )
...
def method_targets_security_platform(
    ...
    *,
    ...
    has_security_surface_signal = context_has_security_surface_signal(context)
    if method_is_read_only(normalized, effective_readonly) and not has_security_surface_signal:
        return False

    operation_type = classify_openclaw_operation(method, context)
    if operation_type in {"chat", "tool_call"}:
        return True
    if has_security_surface_signal:
        return True
...
BLOCKED_REQUEST_NOTICE = "[blocked OpenClaw request content redacted]"
BLOCKED_RESPONSE_NOTICE = "[blocked OpenClaw response content quarantined]"
...
SENSITIVE_REQUEST_KEYS = REQUEST_CONTENT_KEYS | PATH_KEYS | {"raw_input", "request_excerpt", "params_json"}
SENSITIVE_RESPONSE_KEYS = RESPONSE_CONTENT_KEYS | PATH_KEYS | {"raw_response", "raw_response_text"}


def redact_paths_in_text(value: Any) -> str:
    text = str(value or "")
    if not text:
        return ""
    for pattern in (WINDOWS_PATH_RE, UNIX_PATH_RE):
...
def sanitize_payload_fields(payload: Any, *, sensitive_keys: set[str], notice: str) -> Any:
        ...
        for key, value in payload.items():
            ...
            if normalized in sensitive_keys:
                if isinstance(value, dict):
                    ...
                    updated[key] = notice
                continue
            updated[key] = sanitize_payload_fields(value, sensitive_keys=sensitive_keys, notice=notice)
        return updated
    if isinstance(payload, list):
        return [sanitize_payload_fields(item, sensitive_keys=sensitive_keys, notice=notice) for item in payload]
    if isinstance(payload, str):
        return redact_paths_in_text(payload)
    return payload


def sanitize_frame_log_payload(payload: str, *, max_chars: int) -> str:
    parsed = load_json_message(payload)
    if parsed is not None:
        sanitized = sanitize_payload_fields(
            parsed,
            sensitive_keys=SENSITIVE_REQUEST_KEYS | SENSITIVE_RESPONSE_KEYS,
            notice="[redacted]",
        )
        return safe_json_dumps(sanitized, max_chars=max_chars)
    redacted = redact_paths_in_text(payload)
    if len(redacted) <= max_chars:
        return redacted
    return redacted[:max_chars] + "...(已截断)"


def sanitize_openclaw_context_for_quarantine(
    context: dict[str, Any],
    *,
    notice: str = BLOCKED_REQUEST_NOTICE,
) -> dict[str, Any]:
    sanitized = dict(context or {})
    sanitized["input_text"] = notice
    sanitized["turns"] = []
    sanitized["paths"] = []
    return sanitized


def sanitize_blocked_request_payload(
    ...
    *,
    ...
    sanitized_context = sanitize_openclaw_context_for_quarantine(context)
    sanitized_payload = sanitize_payload_fields(
        copy.deepcopy(payload),
        sensitive_keys=SENSITIVE_REQUEST_KEYS,
        notice=BLOCKED_REQUEST_NOTICE,
    )
    if isinstance(sanitized_payload, dict):
        sanitized_payload["__blue_team__"] = {
            "decision": decision,
            "summary": summary,
            "quarantined": True,
        }
    return sanitized_context, safe_json_dumps(sanitized_payload, max_chars=max_chars)


def sanitize_authorization_for_quarantine(authorization: dict[str, Any]) -> dict[str, Any]:
    sanitized = sanitize_payload_fields(
        copy.deepcopy(authorization or {}),
        sensitive_keys=SENSITIVE_REQUEST_KEYS | {"context"},
        notice=BLOCKED_REQUEST_NOTICE,
    )
    if isinstance(sanitized, dict):
        context = sanitized.get("context")
        if isinstance(context, dict):
            sanitized["context"] = sanitize_openclaw_context_for_quarantine(context)
    return sanitized if isinstance(sanitized, dict) else {}


def sanitize_blocked_response_payload(payload: dict[str, Any], *, summary: str, decision: str) -> str:
    sanitized = sanitize_payload_fields(
        copy.deepcopy(payload),
        sensitive_keys=SENSITIVE_RESPONSE_KEYS,
        notice=BLOCKED_RESPONSE_NOTICE,
    )
    if isinstance(sanitized, dict):
...
def extract_openclaw_context(payload: dict[str, Any], *, max_capture_chars: int) -> dict[str, Any]:
    ...
    paths = dedupe_strings(
        [
            ...
            *extract_paths_from_text_values(input_text, turns),
        ]
    )[:20]
...
def sanitize_tool_result_payload(payload: dict[str, Any], *, summary: str, decision: str) -> str:
    return sanitize_blocked_response_payload(payload, summary=summary, decision=decision)
...
class RuntimeBridgeClient:
    ...
    def create_task(
        ...
        *,
        ...
        operation_type = classify_openclaw_operation(method, context)
        stored_context = sanitize_openclaw_context_for_quarantine(context) if context.get("paths") else context
        stored_request_excerpt = "[OpenClaw request payload quarantined]" if context.get("paths") else request_excerpt
        payload = {
            ...
            "params_json": {
                ...
                "content": stored_context["input_text"],
                "turns": stored_context["turns"],
                "skill_names": stored_context["skill_names"],
                "plugin_names": stored_context["plugin_names"],
                "paths": stored_context["paths"],
                "requested_scopes": stored_context["requested_scopes"],
                "source_plugin": stored_context["source_plugin"],
                "target_plugin": stored_context["target_plugin"],
                "mcp_server": stored_context["mcp_server"],
                "capability_name": stored_context["capability_name"],
                "session_id": stored_context["session_id"],
                "approval_id": stored_context["approval_id"],
                "handoff_token": stored_context["handoff_token"],
                "agent_id": stored_context["agent_id"],
                "assistant_agent_id": stored_context["assistant_agent_id"],
                "conversation_id": stored_context["conversation_id"],
                "thread_id": stored_context["thread_id"],
                "run_id": stored_context["run_id"],
                "tool_call_id": stored_context["tool_call_id"],
                "session_key": stored_context["session_key"],
                "workspace_id": stored_context["workspace_id"],
                "assistant_name": stored_context["assistant_name"],
                "assistant_avatar": stored_context["assistant_avatar"],
                "server_version": stored_context["server_version"],
                "default_agent_id": stored_context["default_agent_id"],
                "main_session_key": stored_context["main_session_key"],
                "instance_id": stored_context["instance_id"],
                "client_mode": stored_context["client_mode"],
                "client_version": stored_context["client_version"],
                "role": stored_context["role"],
                "locale": stored_context["locale"],
                "user_agent": stored_context["user_agent"],
                "auth_mode": stored_context["auth_mode"],
                "config_path": stored_context["config_path"],
                "state_dir": stored_context["state_dir"],
                "canvas_host_url": stored_context["canvas_host_url"],
                "connection_id": stored_context["connection_id"],
                "protocol": stored_context["protocol"],
                "gateway_metadata": {
                    "transport": "websocket",
                    "ws_method": method,
                    "ws_call_id": call_id,
                    ...
                    "openclaw_agent_id": stored_context["agent_id"],
                    "openclaw_assistant_agent_id": stored_context["assistant_agent_id"],
                    "openclaw_conversation_id": stored_context["conversation_id"],
                    "openclaw_thread_id": stored_context["thread_id"],
                    "openclaw_run_id": stored_context["run_id"],
                    "openclaw_tool_call_id": stored_context["tool_call_id"],
                    "openclaw_session_key": stored_context["session_key"],
                    "openclaw_workspace_id": stored_context["workspace_id"],
                    "openclaw_assistant_name": stored_context["assistant_name"],
                    "openclaw_server_version": stored_context["server_version"],
                    "openclaw_default_agent_id": stored_context["default_agent_id"],
                    "openclaw_main_session_key": stored_context["main_session_key"],
                    "openclaw_instance_id": stored_context["instance_id"],
                    "openclaw_client_mode": stored_context["client_mode"],
                    "openclaw_role": stored_context["role"],
                    "openclaw_locale": stored_context["locale"],
                    "openclaw_auth_mode": stored_context["auth_mode"],
                    "openclaw_canvas_host_url": stored_context["canvas_host_url"],
                    "request_excerpt": stored_request_excerpt,
                },
            },
        }
...
async def report_local_block(
    ...
    *,
    ...
    try:
        sanitized_context, sanitized_payload = sanitize_blocked_request_payload(
            payload,
            context=context,
            summary=reason,
            decision="deny",
        ...
        task = await asyncio.to_thread(
            ...
            request_excerpt=sanitized_payload,
        )
        await asyncio.to_thread(
            ...
            request_id=request_id,
            method=method,
            input_text=str(sanitized_context.get("input_text") or ""),
            status="failed",
            summary=reason,
            request_excerpt=sanitized_payload,
            raw_response_text=sanitized_payload,
...
async def review_upstream_tool_event(
    ...
    *,
    ...
    try:
        ...
        stored_context = sanitize_openclaw_context_for_quarantine(merged_context) if blocked else merged_context
        stored_response_text = outbound_text if blocked else original_text
        stored_request_excerpt = outbound_text[: config.max_capture_chars] if blocked else request_excerpt
        stored_authorization = sanitize_authorization_for_quarantine(authorization) if blocked else authorization
        await asyncio.to_thread(
            runtime_client.complete,
            task_id=task_id,
            request_id=request_id,
            method=method,
            input_text=str(stored_context.get("input_text") or ""),
            status="failed" if blocked else "done",
            summary=summary,
            request_excerpt=stored_request_excerpt,
            raw_response_text=stored_response_text,
            authorization=stored_authorization,
            metadata={
                "phase": "upstream_event_review",
                "ws_direction": "upstream_to_client",
                "call_id": call_id,
                "tool_call_id": str(stored_context.get("tool_call_id") or ""),
                "operation_type": "tool_result",
                "event_name": event_name,
                "request_args_hash": request_args_hash,
                "session_id": str(stored_context.get("session_id") or ""),
                "approval_id": str(stored_context.get("approval_id") or ""),
                "mcp_server": str(stored_context.get("mcp_server") or ""),
                "capability_name": str(stored_context.get("capability_name") or ""),
                "source_plugin": str(stored_context.get("source_plugin") or ""),
                "target_plugin": str(stored_context.get("target_plugin") or ""),
                "handoff_token": str(stored_context.get("handoff_token") or ""),
                "requested_scopes": list(stored_context.get("requested_scopes") or []),
                "mcp_ticket_key": ticket_key,
                "consume_mcp_ticket": bool(ticket_key),
                "openclaw_event_name": event_name,
                "openclaw_event_type": classify_openclaw_server_event(event_name, stored_context),
            },
        )
...
async def evaluate_runtime_action(
    ...
    *,
    ...
    try:
        ...
        blocked_completion_context, blocked_completion_payload = sanitize_blocked_request_payload(
            payload,
            context=context,
            summary=str(authorization.get("summary") or "OpenClaw request was blocked by platform policy."),
            decision=decision or "deny",
            max_chars=config.max_capture_chars,
        )
        blocked_completion_authorization = sanitize_authorization_for_quarantine(authorization)
        ...
        if decision == "deny":
            ...
            await asyncio.to_thread(
                ...
                request_id=request_id,
                method=method,
                input_text=str(blocked_completion_context.get("input_text") or ""),
                status="failed",
                summary=message,
                request_excerpt=blocked_completion_payload,
                raw_response_text=blocked_completion_payload,
                authorization=blocked_completion_authorization,
                metadata={
                    "phase": "authorize",
                    "blocked_by": "policy",
                    "call_id": call_id,
                    "tool_call_id": blocked_completion_context["tool_call_id"],
                    "operation_type": operation_type,
                    "request_args_hash": request_args_hash,
                    "session_id": blocked_completion_context["session_id"],
                    "mcp_server": blocked_completion_context["mcp_server"],
                    "capability_name": blocked_completion_context["capability_name"],
                    "requested_scopes": blocked_completion_context["requested_scopes"],
                },
            )
...
async def run_bridge(config: BridgeConfig) -> int:
    ...
    def write_frame_log(direction: str, frame_type: str, payload: str) -> None:
        if config.log_jsonl_path is None:
            return
        stored_payload = sanitize_frame_log_payload(payload, max_chars=config.max_capture_chars)
        line = json.dumps(
            {
                "time": now_text(),
                "direction": direction,
                                ...
                                if method:
                                    ...
                                    if method in config.block_methods:
                                        ...
                                        if payload.get("id") is not None:
                                            await websocket.send_text(build_json_rpc_error(payload, OPENCLAW_BLOCKED_USER_MESSAGE))
                                            continue
                                        await websocket.close(code=4403, reason=OPENCLAW_BLOCKED_USER_MESSAGE)
                                        return
