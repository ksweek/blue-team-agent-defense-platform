...
以下是仍然存在的真实边界，不再伪装成“已完成能力”：

- Worker 目前仍基于数据库轮询，不是 Redis / RabbitMQ 队列调度。
- 运行中任务的暂停 / 取消仍是协作式中断，不是对上游请求的强制硬中断。
- 报告已支持 JSON / HTML / DOCX，但更正式的 PDF 报告与更强的横向对比仍可继续补齐。
- 生产环境必须先替换默认密码、默认令牌和默认密钥，再执行 `schema` 初始化并启动服务。

## 发布前建议

如果你准备把这个仓库公开到 GitHub，至少先完成这几件事：

1. 替换所有默认安全参数与示例凭据。
2. 清理本地运行数据、数据库、报告、备份与日志。
3. 用 `scripts/validate.ps1` 或 CI 工作流重新验证。
4. 明确 License、维护者、漏洞通报渠道与版本发布策略。
